...
import { 
  ...
  ChevronLeft, ChevronRight, Filter, Plus, RotateCcw, Repeat
} from 'lucide-react';
...
export const CalendarBulkWizard: React.FC<Props> = ({ onClose, onSuccess, preselectedDates = [] }) => {
  ...
  const [isCustomTimes, setIsCustomTimes] = useState(false); // NEW: Manual Time Toggle
  ...
  // Update times when profile changes (ONLY if not using custom times)
  useEffect(() => {
    if (!isCustomTimes && eventType === 'SHOW' && selectedShowId && selectedProfileId) {
  ...
  }, [selectedShowId, selectedProfileId, eventType, isCustomTimes]);
  ...
  const calendarGrid = useMemo(() => {
    ...
    const dutchStartDay = startDay === 0 ? 6 : startDay - 1; 

    const days = [];
    for (let i = 0; i < dutchStartDay; i++) days.push(null);
  ...
  const bookingConflicts = useMemo(() => {
    ...
    targetDates.forEach(date => {
        const bookings = allReservations.filter(r => r.date === date && r.status !== BookingStatus.CANCELLED && r.status !== BookingStatus.ARCHIVED);
  ...
  const handleSubmit = async () => {
    ...
    targetDates.forEach(date => {
        ...
        const baseEvent = {
            ...
            times: { start: times.start, doorsOpen: times.door, end: times.end }
        };
    ...
    logAuditAction('BULK_CREATE_EVENTS', 'CALENDAR', 'MULTIPLE', { description: `Created ${eventsToCreate.length} events` });
        ...
        <div className="w-full lg:w-64 space-y-4 shrink-0">
            <div className="p-4 bg-slate-900 border border-slate-800 rounded-xl space-y-4">
                <h4 className="text-xs font-bold text-amber-500 uppercase tracking-widest flex items-center">
                    <Repeat size={14} className="mr-2"/> Herhalend Patroon
                </h4>
            ...
            <div className="p-4 bg-slate-900 border border-slate-800 rounded-xl flex flex-col items-center justify-center">
                ...
                {selectedDates.size > 0 && (
                    <button onClick={() => setSelectedDates(new Set())} className="text-[10px] text-red-400 hover:text-red-300 mt-2 underline flex items-center">
                    ...
                    {calendarGrid.map((day, idx) => {
                        ...
                        return (
                            <div 
                                ...
                                className={`aspect-square rounded-lg flex flex-col items-center justify-center cursor-pointer transition-all relative border ${isSelected ? 'bg-amber-500 text-black border-amber-600' : 'bg-transparent border-slate-800/50 text-slate-400 hover:bg-slate-800'}`}
                            >
                                <span className="text-sm font-bold">{day.date.getDate()}</span>
                                {hasEvent && <div className={`w-1.5 h-1.5 rounded-full mt-1 ${isSelected ? 'bg-black' : 'bg-red-500'}`} />}
                            </div>
                        );
                    })}
                </div>
    ...
    return (
        <div className="space-y-6 animate-in fade-in">
            ...
            <div className="grid grid-cols-2 gap-4">
                <div onClick={() => setEventType('SHOW')} className={`p-4 rounded-xl border cursor-pointer transition-all ${eventType === 'SHOW' ? 'bg-purple-900/20 border-purple-500 text-purple-200' : 'bg-slate-900 border-slate-800 text-slate-500'}`}>
                    <div className="font-bold mb-1">Voorstelling</div>
                    <div className="text-xs opacity-70">Publiek event met tickets</div>
                </div>
                <div onClick={() => setEventType('BLACKOUT')} className={`p-4 rounded-xl border cursor-pointer transition-all ${eventType === 'BLACKOUT' ? 'bg-red-900/20 border-red-500 text-red-200' : 'bg-slate-900 border-slate-800 text-slate-500'}`}>
                    ...
                    <div className="space-y-4">
                           ...
                           <div className="space-y-1.5">
                              ...
                              <select 
                                  ...
                                  onChange={(e) => {
                                      ...
                                      if (show && show.profiles.length > 0) setSelectedProfileId(show.profiles[0].id);
                                  }}
                              >
                        ...
                        <div className="pt-2 border-t border-slate-800">
                           <label className="flex items-center space-x-2 cursor-pointer mb-4">
                             <input type="checkbox" checked={isCustomTimes} onChange={(e) => setIsCustomTimes(e.target.checked)} className="rounded bg-slate-800 border-slate-700" />
                             <span className="text-sm font-bold text-slate-300">Handmatige tijden (Override)</span>
                           </label>

                           <div className={`grid grid-cols-3 gap-4 ${!isCustomTimes ? 'opacity-50 pointer-events-none' : ''}`}>
    ...
    return (
        <div className="space-y-6 animate-in fade-in">
            <div className="grid grid-cols-2 gap-4 text-center">
                ...
                <div className="p-6 bg-slate-900 rounded-xl border border-slate-800">
                    <p className="text-xs font-bold text-slate-500 uppercase">Conflicten</p>
            ...
            {Object.keys(bookingConflicts).length > 0 && (
                <div className="p-4 bg-red-900/20 border border-red-500/50 rounded-xl space-y-4 shadow-lg">
                    <div className="flex items-start space-x-3 text-red-500">
                        <ShieldAlert size={32} />
                        <div>
                            <h4 className="font-bold text-base">LET OP: Actieve Reserveringen!</h4>
                            <p className="text-sm text-red-200/80 mt-1">Op <strong>{Object.keys(bookingConflicts).length}</strong> data zijn er reserveringen.</p>
                        </div>
                    </div>
                    <label className="flex items-center space-x-3 p-3 bg-black/40 rounded-lg cursor-pointer border border-red-900/50">
                        <input type="checkbox" checked={forceBookings} onChange={(e) => setForceBookings(e.target.checked)} className="rounded bg-slate-800 border-red-500 text-red-500" />
            ...
            {eventConflicts.length > 0 && (
                <div className="p-4 bg-amber-900/20 border border-amber-900/50 rounded-xl space-y-4">
                    ...
                    <label className="flex items-center space-x-3 p-3 bg-black/20 rounded-lg cursor-pointer hover:bg-black/40 transition-colors">
                        <input type="checkbox" checked={overwrite} onChange={(e) => setOverwrite(e.target.checked)} className="rounded bg-slate-800 border-amber-500/50 text-amber-500" />
                        <span className="text-sm text-amber-100">Overschrijf bestaande events</span>
                    </label>
                </div>
            )}
        </div>
  ...
  return (
    <div className="h-full flex flex-col bg-slate-950">
        <div className="px-6 py-4 bg-black/20 border-b border-slate-900"><Stepper steps={STEPS} current={step} /></div>
        ...
        <div className="p-6 border-t border-slate-900 bg-slate-900/50 flex justify-between items-center">
            <Button variant="ghost" onClick={step === 0 ? onClose : () => setStep(s => s-1)}>{step === 0 ? 'Annuleren' : 'Vorige'}</Button>
            {step < 2 ? (
                <Button onClick={() => setStep(s => s+1)} disabled={selectedDates.size === 0}>Volgende <ArrowRight size={16} className="ml-2"/></Button>
            ) : (
                <Button onClick={handleSubmit} disabled={isProcessing || (Object.keys(bookingConflicts).length > 0 && !forceBookings)} className="bg-emerald-600 hover:bg-emerald-700 min-w-[150px]">
